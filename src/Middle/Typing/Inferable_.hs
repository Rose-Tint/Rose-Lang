...
inferStmt env (Reassignment _name val) = infer env val
inferStmt env (Return val) = infer env val
inferStmt env (ValStmt val) = infer env val
inferStmt _ Break = do
    tv <- fresh
    return (nullSubst, tv)
inferStmt _ Continue = do
    tv <- fresh
    return (nullSubst, tv)
inferStmt _ NullStmt = do
    tv <- fresh
    return (nullSubst, tv)
