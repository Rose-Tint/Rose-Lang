...
insertType :: SymbolData -> SymbolTable -> SymbolTable
{-# INLINABLE insertType #-}
insertType dta tbl = tbl
    { tblTypes = insert dta (tblTypes tbl) }


insertTrait :: SymbolData -> SymbolTable -> SymbolTable
{-# INLINABLE insertTrait #-}
insertTrait dta tbl = tbl
    { tblTraits = insert dta (tblTraits tbl) }


insertGlobal :: SymbolData -> SymbolTable -> SymbolTable
{-# INLINABLE insertGlobal #-}
insertGlobal dta tbl = tbl
    { tblGlobals = insert dta (tblGlobals tbl) }


insertScoped :: SymbolData -> SymbolTable -> SymbolTable
{-# INLINABLE insertScoped #-}
insertScoped dta tbl = let insert' = insert dta in
